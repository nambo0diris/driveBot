    ...
    try {
        ...
        await newDBconnect.getOrderInfo(async (result) => {
            await convert_to_jpeg(result, "original").then(async () => {
                return await ctx.replyWithDocument({ source: `E:///myProjects/driveBot/temp/users/${ctx.message.chat.id}/Полный_разворот_1.jpg` });
            });
            await convert_to_jpeg(result, "original").then(async () => {
                return await ctx.replyWithDocument({ source: `E:///myProjects/driveBot/temp/users/${ctx.message.chat.id}/Полный_разворот_2.jpg` });
            });
            await convert_to_jpeg(result, "original").then(async () => {
                return await ctx.replyWithDocument({ source: `E:///myProjects/driveBot/temp/users/${ctx.message.chat.id}/Короткая_версия.jpg` });
            });
            if(ctx.state.prava === "+европейские"){
                await convert_to_jpeg(result, "original").then(async () => {
                    return await ctx.replyWithDocument({ source: `E:///myProjects/driveBot/temp/users/${ctx.message.chat.id}/Европейские(на пластик)_1.jpg` });
                });
                await convert_to_jpeg(result, "original").then(async () => {
                    return await ctx.replyWithDocument({ source: `E:///myProjects/driveBot/temp/users/${ctx.message.chat.id}/Европейские(на пластик)_2.jpg` });
                });
            }
